...
public class JsonViewSerializer extends JsonSerializer<JsonView> {
  private static final Logger log = LogManager.getLogger(JsonViewSerializer.class);
...
//      JsonSerializer serializer = serializerProvider.findValueSerializer(obj.getClass());
...
//      }
    ...
    boolean writeMap(Object obj) throws IOException {
      if(obj instanceof Map) {
        ...
        for(Map.Entry<Object, Object> entry : map.entrySet()) {
          ...
          new JsonWriter(jgen, result, currentMatch, serializerProvider).write(null, entry.getValue());
        }
        jgen.writeEndObject();
      } else {
        return false;
    ...
    void write(String fieldName, Object value) throws IOException {
      ...
      if(!writePrimitive(value) && !writeEnum(value) && !writeList(value) && !writeMap(value) && !writeSpecial(value)) {
        writeObject(value);
      }
